...
class Net(nn.Module):
    ...
    def multi_l2_norm(self,input):
        ...
        _output = torch.div(input, norm.view(input.size(0),-1, 1).expand_as(input))
...
class ResNetLike(nn.Module):
    def __init__(self):
        ...
        #
        # self.feat_extractor.add_module('ResBlock' + str(3),
        #                                ResBlock(num_planes[3], num_planes[3 + 1]))
        # self.feat_extractor.add_module('MaxPool' + str(3),
        #                                nn.MaxPool2d(kernel_size=2, stride=2, padding=0))
